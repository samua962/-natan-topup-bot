const express = require("express");
const cors = require("cors");
const bot = require("../bot/bot");  // Import bot

const categoryRoutes = require("./routes/category.routes");
const productRoutes = require("./routes/product.routes");
const orderRoutes = require("./routes/order.routes");
const settingsRoutes = require("./routes/settings.routes");
const authRoutes = require("./routes/auth.routes");
const usersRoutes = require("./routes/users.routes");
const giveawayRoutes = require("./routes/giveaway.routes");
const { router: webhookRouter, setBotInstance } = require("./routes/webhook.routes");

const app = express();

// Pass bot instance to webhook routes
setBotInstance(bot);

app.use(cors());

// IMPORTANT: Order matters!
// First, handle Telegram webhook (needs raw body for bot)
app.post('/webhook/telegram', express.raw({ type: 'application/json' }), (req, res) => {
    bot.handleUpdate(req.body, res);
});

// Then ShegerPay webhook
app.use('/webhook/shegerpay', express.json({
    verify: (req, res, buf, encoding) => {
        req.rawBody = buf.toString();
    }
}), webhookRouter);

// Then regular JSON for API routes
app.use(express.json());

// Public routes
app.use("/api/auth", authRoutes);

// Auth middleware for protected routes
const authMiddleware = (req, res, next) => {
    const token = req.headers.authorization?.split(" ")[1];
    
    if (!token) {
        return res.status(401).json({ error: "Unauthorized" });
    }
    
    try {
        const jwt = require("jsonwebtoken");
        const decoded = jwt.verify(token, process.env.JWT_SECRET || "natan_topup_secret_key_2024");
        req.user = decoded;
        next();
    } catch (error) {
        res.status(401).json({ error: "Invalid token" });
    }
};

// Protected routes
app.use("/api/categories", authMiddleware, categoryRoutes);
app.use("/api/products", authMiddleware, productRoutes);
app.use("/api/orders", authMiddleware, orderRoutes);
app.use("/api/settings", authMiddleware, settingsRoutes);
app.use("/api/users", authMiddleware, usersRoutes);
app.use("/api/giveaway", authMiddleware, giveawayRoutes);

module.exports = app;