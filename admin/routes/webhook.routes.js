...
// Webhook endpoint for ShegerPay
router.post("/shegerpay", async (req, res) => {
    ...
    if (event === "payment.verified" || (req.body.event === "payment.verified")) {
        ...
        if (pendingOrder) {
            ...
            if (isInstant) {
                ...
                if (ragnerResult && ragnerResult.success) {
                    ...
                    // Delivery failed, but payment verified – set to APPROVED
                    await db.query(
                ...
                // For manual products, auto-approve (admin still needs to complete)
                await db.query(
                    `UPDATE orders SET status = 'APPROVED', verified_by_shegerpay = true WHERE id = $1`,
                    [pendingOrder.id]
                );
                ...
                // Build admin message with order details
